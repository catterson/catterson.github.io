...
### ![](images/clipboard-2826245380.png)
...
-   **Hypotheses :**
    ...
    -   **Null Hypothesis :** more (or no difference) in power words among authors of color than white authors.

    -   **Alternative Hypothesis :** fewer power words among authors of color than white authors.
...
boxplot(power ~ pocuF, data = d)
par(mfrow = c(1,3))
plotmeans(power ~ pocuF, data = d)
plotmeans(power ~ pocuF, data = d, ylim = c(range(d$power, na.rm = T)))
msd <- c(mean(d$power) - sd(d$power), mean(d$power) + sd(d$power))
plotmeans(power ~ pocuF, data = d, ylim = c(msd))
## what do you see? what's effective about the graph? what's confusing / misleading???
```

-   Notes on the graph :

    -   the good : included the sample size of each group; there appears to be a "BIG" difference between white authors and authors of color in terms of power words.

    -   the bad : zooming in exaggerates the difference....
...
-   **The Model :** $\hat{Y} = a + b_1 * X_1$
    ...
    -   $X_1$ = A **dummy coded** categorical variable. This is often the default method of comparing two groups, and compares each level of the factor to a reference level.
        -   $X_1$ = 0 = not an author of color

        -   $X_1$ = 1 = are an author of color
    -   $b_1$ = the slope = the predicted change in Y as X changes by 1

-   **Using the Model to Make Predictions**

    -   power words = 3.56 - .49 \* Author of Color

        -   if author of color = "yes", X = 1 = 3.56 - .49 \* 1 = 3.07 = the intercept + the adjustment we make for this group.

        -   if author of color = "no", X = 0 = 3.56 - .49 \* 0 = 3.56 = the intercept
...
-   $R^2$ **(Effect Size) :** This describes the percentage of variation in hunger that is explained by our model (in this case, using whether someone had breakfast to make predictions of hunger.) So, using the model to make predictions of hunger reduces our squared errors by %17 (vs. using the mean to make predictions.)
    -   we see a very small $$R^2$$ value.
        -   This tells us : RACE DOESN'T PREDICT VERY MUCH POWER USAGE IN ABSTRACTS. BUT NON-ZERO!!!!
        -   This does NOT tell us :
            -   whether this difference is "statistically significant" or not (i.e., is the difference consistent, or do we think that it's there JUST because of sampling error????)
            -   whether the effect is important or matters : any biases in the field matter.

```{r}
summary(mod)$r.squared*100 # our model (using race to predict power usage) explains about .62% of the variation in power words used.
```
...
-   **Cohen's D** = slope / pooled(sd)
-   The psych package has a cohen's d function built in that calculates this for you.

```{r}
library(psych)
cohen.d(power ~ pocuF, data = d)
#cohen.d(d$DV, d$IV)
#cohen.d(DV ~ IV, data = d)
```

Okay, so what does this statistic mean? Well, this tells us that the difference in power words between White Authors and Authors of Color (the slope in our model = -.49) is only .17 times the difference between any two random individuals (the pooled sd). But is this a little? A lot?
...
range(bucket)
se <- sd(bucket) # the estimate of sampling error
b <- coef(mod)[2] # the original slope

b + 1.96 * se # upper limit of 95% CI
b - 1.96 * se # lower limit of 95% CI

hist(bucket)
abline(v = mean(bucket), lwd = 5)
abline(v = b + 1.96 * se, lwd = 2)
abline(v = b - 1.96 * se, lwd = 2)
```

### BRINGING THIS ALL TOGETHER WTF IS GOING ON?!??!?!?!?!!?!?

We found a difference in the power words that White Authors and Authors of Color used in their abstracts ([b = -.49]{.underline}, cohen's d = .17, $$R^2$$ = .0062, 95% CI = \[-.77, -.22\]).

-   QUESTION : what statistic (cohen's d; R\^2, or the 95% CI) seems the most relevant for understanding this difference (b = -.49)????

    -   **95% CI =** tells us the difference is "significant"...not just due to sampling error.

        -   as a researcher we want the CI to be narrow but tells us that our study is (maybe) a more accurate guess about what is going on with the "true" population.

        -   the 95% CI tells us nothing about the effect size.
...
|                                                           |                                                                                                                                                                                                                                                                                                                                                                        |                                                                                                                                    |
|                                                           |                                                                                                                                                                                                                                                                                                                                                                        | 1.  the variance in our sample (with the idea that the more individuals vary in the sample, the more sampling error we might have) |
|                                                           |                                                                                                                                                                                                                                                                                                                                                                        |                                                                                                                                    |
